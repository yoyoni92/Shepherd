...
def test_vehicle_type_round_trips_and_patches(client):
    ...
    created = client.post(
        "/vehicles", headers=H(),
        json={"licensing_plate": plate, "vehicle_type": "car", "current_km": 1000},
    )
    ...
    patched = client.patch(
        f"/vehicles/{vid}", headers=H(), json={"vehicle_type": "truck", "current_km": 4200}
    )
...
def test_patch_driver_and_customer(client):
    drv = client.post(
        "/drivers", headers=H(), json={"full_name": "א", "phone_number": _plate()}
    ).json()
    upd = client.patch(
        f"/drivers/{drv['driver_id']}",
        headers=H(),
        json={"status": "inactive", "license_valid_to": "2028-01-01"},
    )
    ...
    cupd = client.patch(
        ...
        headers=H(),
        json={"email": "x@y.co.il", "status": "inactive"},
    )
...
def test_delete_customer_nulls_vehicle_link(client):
    ...
    veh = client.post(
        "/vehicles", headers=H(), json={"licensing_plate": _plate(), "customer_id": cid}
    ).json()
...
def test_invalid_vehicle_type_rejected(client):
    ...
    resp = client.post(
        "/vehicles", headers=H(), json={"licensing_plate": _plate(), "vehicle_type": "spaceship"}
    )
