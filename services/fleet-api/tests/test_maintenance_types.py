...
def test_create_list_update_maintenance_type(client):
    ...
    patched = client.patch(
        f"/maintenance-types/{tid}",
        headers=H(),
        json={"interval_km": 8000, "steps": ["א", "ב", "ג"]},
    )
...
def test_validation_rejects_bad_input(client):
    ...
    assert client.post(
        "/maintenance-types", headers=H(), json={**base, "steps": []}
    ).status_code == 422
    assert client.post(
        "/maintenance-types", headers=H(),
        json={**base, "name": _name(), "steps": ["קטן", "קטן"]},
    ).status_code == 422
    assert client.post(
        "/maintenance-types", headers=H(),
        json={"name": _name(), "interval_km": 0, "steps": ["קטן"]},
    ).status_code == 422
...
def test_rejects_when_no_interval(client):
    ...
    bad_months = client.post(
        "/maintenance-types", headers=H(),
        json={"name": _name(), "interval_months": 0, "steps": ["קטן"]},
    )
...
def test_delete_blocked_when_in_use(client):
    mt = client.post(
        "/maintenance-types", headers=H(),
        json={"name": _name(), "interval_km": 10000, "steps": ["קטן", "גדול"]},
    ).json()
    plate = str(uuid.uuid4().int % 100_000_000).zfill(8)
    client.post(
        "/vehicles", headers=H(), json={"licensing_plate": plate, "maintenance_type_id": mt["id"]}
    )
    ...
    unused = client.post(
        "/maintenance-types", headers=H(),
        json={"name": _name(), "interval_km": 5000, "steps": ["יחיד"]},
    ).json()
...
def test_forbidden_for_driver(client):
    resp = client.get(
        "/maintenance-types", headers=driver_headers("00000000-0000-0000-0000-000000000000")
    )
