...
def test_refresh_kpi_daily_row_math(conn):
    ...
    customer_id = _scalar(
        conn,
        "INSERT INTO customers (full_name) VALUES ('Acme') RETURNING customer_id",
    )
    for name, phone in [("D1", "+972500000001"), ("D2", "+972500000002")]:
        conn.execute(
            text("INSERT INTO drivers (full_name, phone_number) VALUES (:n, :p)"),
            {"n": name, "p": phone},
        )
    ...
    v1 = _scalar(
        conn,
        """INSERT INTO vehicles (licensing_plate, customer_id, current_km, next_maintenance_km,
                                 insurance_valid_to, license_valid_to)
           VALUES ('V-1', :c, 10000, 9000, :ins, :lic) RETURNING vehicle_id""",
        c=customer_id, ins=today + timedelta(days=5), lic=today + timedelta(days=400),
    )
    ...
    v2 = _scalar(
        ...
        """INSERT INTO vehicles (licensing_plate, customer_id, current_km, next_maintenance_km,
                                 insurance_valid_to, license_valid_to)
           VALUES ('V-2', :c, 5000, 20000, :ins, :lic) RETURNING vehicle_id""",
        c=customer_id, ins=today + timedelta(days=400), lic=today + timedelta(days=400),
    )
    ...
    for vid, km, ts in km_rows:
        conn.execute(
            text("""INSERT INTO km_updates (vehicle_id, km, recorded_ts, source)
                    VALUES (:v, :km, :ts, 'admin_ui')"""),
            {"v": vid, "km": km, "ts": ts},
        )
    ...
    for sd, km in [(today - timedelta(days=40), 800), (today - timedelta(days=10), 1200)]:
        conn.execute(
            text("""INSERT INTO vehicle_care (vehicle_id, service_date, maintenance_type, km_at_service)
                    VALUES (:v, :sd, 'small', :km)"""),
            {"v": v1, "sd": sd, "km": km},
        )
    ...
    assert row["maintenance_due_count"] == 1               # v1
